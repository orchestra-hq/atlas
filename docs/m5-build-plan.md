...
**Phase 1c — scaffold.** Stand up the Starlight project under `website/` (pinned versions), set `site`

- `base: '/atlas'`, add a landing page and the top-level sidebar groups. Add
  `.github/workflows/docs-site.yml`: PR job builds the site + runs a link check on changes under
  `website/`; `main` job deploys via `actions/deploy-pages`. Add `website/node_modules` and the Astro
