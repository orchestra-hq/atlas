...
Current recommendation: **hybrid (b)+(c)** — containers where a runtime is present (most CUDA fleets), managed runtimes as the fallback (bare metal, macOS/MLX, air-gapped). The worker probes and picks; the operator can force either. Decide at M0 build time — M0's engine pair (llama.cpp + vLLM) needs both paths anyway: llama.cpp favors (c), vLLM favors (b).
