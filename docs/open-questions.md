...
**Open:** stand up the acceptance run — (a) register a self-hosted CUDA runner (doubles as dogfooding, per [m0-build-plan.md](m0-build-plan.md#testing-tiers)), then add a workflow that runs the harness with `--engine vllm` + the Qwen parser flags **and** `CONF_CLAUDE_CODE_SMOKE=1` on the sonnet alias; or (b) defer to M1 cloud fleets. Until then the dedicated **Claude Agent SDK package** (`claude-agent-sdk`, model-initiated custom-tool loop) is also a capable-tier client — the small model emits its MCP tool calls as prose, so the per-PR `agent-sdk` cell uses the forced Anthropic-SDK loop instead; the dedicated-package run belongs in the same acceptance pass.
