...
- An in-memory queue means a control-plane restart drops not-yet-dispatched queued requests; this is acceptable (they are un-acked, clients retry). Durable/HA admission is deferred to a later HA-control-plane milestone.
