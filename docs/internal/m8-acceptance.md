...
**Wired (greens on the nightly run).** `scripts/conformance-m8.sh` is engine-parameterized (`CONF_M8_ENGINE`/`CONF_M8_MODEL`/`CONF_M8_ENGINE_ARGS`/`CONF_M8_READY_TRIES`/`CONF_M8_REQUIRE`), so the _same_ G23 gate runs per-PR on llama.cpp/CPU and on the **vLLM** and **SGLang** tracks of [nightly-gpu.yml](../../.github/workflows/nightly-gpu.yml) with a non-catalog safetensors qwen3 repo (`Qwen/Qwen3-8B`) and each track's existing GPU-fit tuning. On those engines the script additionally asserts the auto-config line carries `--tool-call-parser …` — proving the family→parser map engaged (the dimension the CPU tier cannot show). SGLang is scoped to G1–G4 (no G9, matching the G17 SGLang cell: a 7–8B model drives the multi-call agent loop only intermittently; G3 tool use is the agent-critical proof). Since these run on the owner-gated GPU nightly (`GPU_ACCEPTANCE_ENABLED`), they prove out on the next nightly run, not per-PR. **Still a follow-on:** **MLX** (Apple Silicon) auto-config — its families are template-driven (no parser flags, the same signal as the CPU tier), so it adds the least new signal and is left for a later nightly addition. Tracked in [open-questions.md](open-questions.md).
