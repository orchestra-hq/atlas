...
> **✅ Accepted — M8 declared done 2026-06-29.** The per-PR `Conformance (M8)` job is green on `main`: **G23** proves `atlas up --model <hugging-face-repo>` for a known-family repo with **no catalog row** auto-configures from the model's own metadata and serves an **agent-grade** endpoint, end to end on a real llama.cpp deployment, via [`scripts/conformance-m8.sh`](../../scripts/conformance-m8.sh). Introduced in [#136](https://github.com/orchestra-hq/atlas/pull/136) and stabilized in [#138](https://github.com/orchestra-hq/atlas/pull/138) (skip the redundant, flake-prone anthropic-ts subset — the required groups are fully covered by the pytest Python-SDK + agent-SDK suites). Combined with the per-PR Go unit/integration gates for the M8 resolution logic, all six M8 phase exit criteria are met. The GPU-engine breadth below was a non-blocking follow-on and is now also proven green ([run 28382726905](https://github.com/orchestra-hq/atlas/actions/runs/28382726905): vLLM + SGLang auto-derive the family parser flags for a catalog-less `Qwen/Qwen3-8B`). The criteria below stay the definition of done.
...
A breadth dimension layered on the per-PR proof, analogous to M2's full capability matrix and M3's prefix-cache-reuse tiering: the **parser-flag** dimension of auto-config (`--tool-call-parser hermes`/`qwen25`/… and the reasoning parsers families set on **vLLM/SGLang**) is invisible on llama.cpp, which drives tool-calling from the chat template, so the per-PR CPU tier proves classification + reasoning gating + sampling + context but not the parser flags. Observing auto-config on a parser-flag engine (vLLM/SGLang) validates the family map's per-engine args beyond llama.cpp. This grows on the nightly tier and does **not** gate M8-done, because the phase exit criteria are each met end-to-end on the real per-PR deployment.

**Proven on the nightly (green).** `scripts/conformance-m8.sh` is engine-parameterized (`CONF_M8_ENGINE`/`CONF_M8_MODEL`/`CONF_M8_ENGINE_ARGS`/`CONF_M8_READY_TRIES`/`CONF_M8_REQUIRE`), so the _same_ G23 gate runs per-PR on llama.cpp/CPU and on the **vLLM** and **SGLang** tracks of [nightly-gpu.yml](../../.github/workflows/nightly-gpu.yml) with a non-catalog safetensors qwen3 repo (`Qwen/Qwen3-8B`) and each track's existing GPU-fit tuning. On those engines the script additionally asserts the auto-config line carries `--tool-call-parser …` — proving the family→parser map engaged (the dimension the CPU tier cannot show). SGLang is scoped to G1–G4 (no G9, matching the G17 SGLang cell: a 7–8B model drives the multi-call agent loop only intermittently; G3 tool use is the agent-critical proof). These run on the owner-gated GPU nightly (`GPU_ACCEPTANCE_ENABLED`). **Both passed first run** ([run 28382726905](https://github.com/orchestra-hq/atlas/actions/runs/28382726905), 2026-06-29): vLLM auto-derived `--enable-auto-tool-choice --tool-call-parser hermes --reasoning-parser qwen3` (gates G1–G4,G9 green) and SGLang `--tool-call-parser qwen25 --reasoning-parser qwen3` (gates G1–G4 green) for the catalog-less `Qwen/Qwen3-8B`, alongside the green catalog acceptance on each engine — so auto-config is proven agent-grade across the GPU engines, not just llama.cpp. **Still a follow-on:** **MLX** (Apple Silicon) auto-config — its families are template-driven (no parser flags, the same signal as the CPU tier), so it adds the least new signal and is left for a later nightly addition. Tracked in [open-questions.md](open-questions.md).
