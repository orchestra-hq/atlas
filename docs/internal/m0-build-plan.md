...
Phase 8 was split for hardware reasons, by the project owner's call: the **code** (vLLM adapter, uv runtime provisioning, `atlas up --engine`) landed sharing the core⇄OpenAI translation with llama.cpp via `internal/engines/openaichat` so both engines produce one set of semantics, with unit tests for the vLLM-only endpoints (token count, context window). What was **deliberately not done this phase** was the nightly/GPU conformance run that the original exit criterion (_all groups green on both engines_) implies: that is the full-matrix tier (see [Testing tiers](#testing-tiers)), which needs a CUDA runner — vLLM does not run on the CPU PR runner. So the per-PR gate stayed llama.cpp `G1–G8`, and "both engines green" was asserted by construction (shared translation + unit tests) until the GPU run could observe it end-to-end. **That run now exists and is green** — an on-demand CUDA runner (`machulav/ec2-github-runner`) on a scheduled nightly; the full acceptance run on vLLM landed in phase 11 and closed M0 (see [m0-acceptance.md](m0-acceptance.md)).
