...
| `POST /v1/embeddings`       | Shipped (M3 phase 2a): OpenAI embeddings shape (`input` as string or array → indexed float vectors + usage), served by wrapping the engine's embedding task; routes only to an `embedding`-class model, a chat model is a clean 400 — [ADR-0012](decisions/0012-embeddings-and-reranker-model-classes.md)                                       |
| `POST /v1/rerank`           | Shipped (M3 phase 2b): native Atlas endpoint following the de-facto Cohere rerank shape (query + documents + top_n → relevance-ordered results, optional `return_documents`); routes only to a `reranker`-class model, others a clean 400; no OpenAI equivalent to mirror — [ADR-0012](decisions/0012-embeddings-and-reranker-model-classes.md) |
| `GET /v1/models`            | Shipped: single handler shared with the Anthropic surface, returning the Anthropic list shape (`data[].type`, `has_more`, `first_id`/`last_id`); lenient OpenAI SDKs read `data[].id` fine                                                                                                                                                      |
| `POST /v1/completions`      | Legacy text completion — **not implemented** (low priority); no route is registered                                                                                                                                                                                                                                                             |
