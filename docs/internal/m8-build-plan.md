...
> **✅ M8 done (declared 2026-06-29).** Ratified 2026-06-28: the owner accepted [ADR-0015](decisions/0015-bring-any-model-auto-configuration.md) and scheduled M8 **ahead of M6/M7** (the five open questions are resolved — see below). Phases 1–5 are built and merged as green-only PRs (`atlas inspect` → family map + serve → fit gate → warn-and-serve middle case → the conformance gate). The per-PR `Conformance (M8)` job (**G23**) is green on `main`, and the vLLM + SGLang GPU breadth is proven on the nightly — the auto-config path serves agent-grade for a catalog-less known-family repo across engines. Result + evidence: [m8-acceptance.md](m8-acceptance.md). M8 refines the M8 milestone in [roadmap.md](../roadmap.md).
