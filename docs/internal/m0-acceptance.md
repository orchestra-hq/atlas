...
> **✅ Accepted — M0 declared done 2026-06-25.** The scheduled nightly acceptance run (both engines, on `main` after the vLLM `reasoning`-field fix) is green: **vLLM** (GPU) 50 pass / 0 fail / 3 skip and **llama.cpp** (CPU) 50 pass / 0 fail / 3 skip across G1–G10, with the real Claude Code drop-in (criterion 1) passing on vLLM. The three skips per engine are legitimate and cross-covered. Two are common to both engines: G7 engine-down 529 (a placeholder needing harness lifecycle control) and the G10 token-counts-in-log cell (asserted green per-PR in CI, redundant in the nightly). The third is engine-specific: vLLM skips the G4 non-reasoning-graceful case (one hybrid serves every tier there — it passes on llama.cpp), while llama.cpp skips the Claude Code smoke (a GPU/capable-tier cell, green on vLLM). The criteria below stay the definition of done for the API surface.
