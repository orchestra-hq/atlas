...
> **⏳ Pending.** M3's code is complete (all four phases of [m3-build-plan.md](m3-build-plan.md) shipped — affinity routing, embeddings + reranker classes, the audit log, and cloud-fallback passthrough — with ADRs [0011](decisions/0011-prefix-session-affinity-routing.md)/[0012](decisions/0012-embeddings-and-reranker-model-classes.md)/[0013](decisions/0013-cloud-fallback-passthrough.md) accepted and the logic gated per-PR by Go unit + integration tests). What remains to flip M3 to _done_ is the **end-to-end conformance** the build plan calls for: G19–G22 observed against a real two-process llama.cpp deployment every PR, plus the GPU-engine breadth (genuine prefix-cache reuse, embeddings/rerank on a GPU engine) on the nightly tier. The criteria below are the definition of done; the banner flips when the new conformance job and nightly cells are green.
