...
- **Phase 4b (follow-up) — placement + lifecycle.** VRAM-fit placement, `atlas deploy/scale/stop`, and auto-start + idle-stop, all of which need the server to command a worker to load a model on demand (a new `load`/`unload` wire message + worker-side dynamic engine launch). VRAM-fit _placement_ has no caller until then — there is nothing to place while workers self-declare — so it ships with deploy in 4b. Replica selection in 4a is round-robin; load-aware selection is a 4b refinement.
