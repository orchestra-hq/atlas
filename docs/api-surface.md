...
- **Served-by label (ADR-0013):** every chat response carries an `x-atlas-served-by` header — `local` for the fleet, `cloud` when cloud-fallback spilled the request to an upstream provider. It is out-of-band: the response **body is unchanged**, so the SDK parses it identically; only operators/dashboards read the header. Cloud-served tokens are billed to a distinct usage class (`atlas usage` shows them under a `cloud:<provider>` worker), so spill spend is visible and separable. Cloud-fallback is **off by default** and opt-in per model with operator-supplied upstream credentials.
...
| `POST /v1/embeddings`       | Shipped (M3 phase 2a): OpenAI embeddings shape (`input` as string or array → indexed float vectors + usage), served by wrapping the engine's embedding task; routes only to an `embedding`-class model, a chat model is a clean 400 — [ADR-0012](internal/decisions/0012-embeddings-and-reranker-model-classes.md)                                       |
| `POST /v1/rerank`           | Shipped (M3 phase 2b): native Atlas endpoint following the de-facto Cohere rerank shape (query + documents + top_n → relevance-ordered results, optional `return_documents`); routes only to a `reranker`-class model, others a clean 400; no OpenAI equivalent to mirror — [ADR-0012](internal/decisions/0012-embeddings-and-reranker-model-classes.md) |
...
Admin auth reuses the same API-key system: the `/admin/*` surface requires a key carrying the `admin` scope, not a separate token ([ADR-0008](internal/decisions/0008-control-plane-persistence-and-api-keys.md)). Scoped keys and the `/admin/*` gate land in phase 5 (5a mints admin-scoped keys; 5b enforces the gate).
