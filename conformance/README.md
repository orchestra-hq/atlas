...
The harness is engine-agnostic, so the same suite runs against a vLLM-backed Atlas on a GPU host (the full-matrix tier — not yet wired into CI; see [open-questions.md](../docs/internal/open-questions.md)):
...
This vLLM run on a capable GPU model — with `CONF_CLAUDE_CODE_SMOKE=1` enabling the real Claude Code cell — is the full-matrix acceptance tier that **closed out M0** (declared done 2026-06-25; it now runs green on the scheduled nightly — see [m0-acceptance.md](../docs/internal/m0-acceptance.md)).
