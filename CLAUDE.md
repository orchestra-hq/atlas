...
**M0–M4 done; M5 next.** M0, M0.5, and M1 were declared done 2026-06-25 (M0/M0.5's GPU acceptance green on both engines per `docs/m0-acceptance.md`; M1's multi-host fleet acceptance per `docs/m1-acceptance.md`). **M2 ("operate a real fleet from the terminal") was declared done 2026-06-26** — observability + backpressure proven per-PR, MLX + SGLang engine-breadth acceptance green on real hardware per `docs/m2-acceptance.md`. **M3 ("ecosystem & differentiation deepeners") was declared done 2026-06-26** — affinity routing, embeddings + reranker classes, the audit log, and cloud-fallback passthrough, with the per-PR `Conformance (M3)` job (G19–G22) green on a real two-process llama.cpp deployment per `docs/m3-acceptance.md`. **M4 ("deliverability") was declared done 2026-06-27** — `install.sh`, cosign keyless signing, and a Homebrew tap (formula pushed via a dedicated Release App), proven end-to-end by the `v0.1.0` release run; the public `brew`/`curl` channels light up on the owner's go-live flip (repo public + publish the draft) per `docs/m4-build-plan.md`. M5 (`docs/roadmap.md`: packaging — compose/systemd/k8s manifests + reference IaC) is the next milestone, with M6 (web console) after; full HA + the hosted offering stay deferred. Design truth still lives in `docs/`; when code and a design doc diverge, fix one or the other in the same change.
