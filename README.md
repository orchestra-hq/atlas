...
**Status: M0–M2 done (M2 declared 2026-06-26), M3 next.** M0 ("Claude Code on your own box"), M0.5 ("Release & prove"), M1 ("Fleet"), and M2 ("Operate from the terminal") are complete — M0/M0.5 acceptance is green on both engines with the real Claude Code drop-in proven on a GPU ([docs/m0-acceptance.md](docs/m0-acceptance.md)), M1's multi-host fleet acceptance (G1–G14 across two machines, two engines) is green ([docs/m1-acceptance.md](docs/m1-acceptance.md)), and M2's engine-breadth acceptance (MLX on Apple Silicon + SGLang on GPU) is green alongside the per-PR observability/backpressure gates ([docs/m2-acceptance.md](docs/m2-acceptance.md)). Design truth in `docs/` is the source of truth.
